    ...
    def forward(
        ...
        if labels is not None and d_tags is not None:
            loss_labels = sequence_cross_entropy_with_logits(
                logits_labels, labels, mask, label_smoothing=self.label_smoothing
            )
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for label_namespace in self.label_namespaces:
            ...
            for predictions in predictions_list:
                ...
                tags = [self.vocab.get_token_from_index(x, namespace=label_namespace) for x in argmax_indices]
