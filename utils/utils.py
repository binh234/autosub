...
def get_dtype(audio_format):
    if audio_format.width not in [1, 2, 4]:
        raise ValueError(
            'Unsupported sample width: {}'.format(audio_format.width))
...
def get_timestamp_string(timedelta, format):
    ...
    if '.' in str(timedelta):
        timestamp = "0" + \
            str(timedelta).split(".")[0] + sep + \
            str(timedelta).split(".")[-1][:3]
    else:
...
def write_to_file(output_file_handle_dict, inferred_text, line_count, limits):
    ...
    for format in output_file_handle_dict.keys():
        from_dur = get_timestamp_string(
            datetime.timedelta(seconds=float(limits[0])), format)
        to_dur = get_timestamp_string(
            datetime.timedelta(seconds=float(limits[1])), format)
